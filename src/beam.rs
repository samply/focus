...
use beam_lib::{BeamClient, BlockingOptions, MsgId, RawString, TaskRequest, TaskResult};
...
static BEAM_CLIENT: Lazy<BeamClient> = Lazy::new(|| {
    BeamClient::new(
        ...
        CONFIG
            ...
            .to_string()
            .parse()
            .expect("Uri always converts to url"),
    )
});
...
pub async fn retrieve_tasks() -> Result<Vec<TaskRequest<String>>, FocusError> {
    ...
    BEAM_CLIENT
        .poll_pending_tasks::<RawString>(&block)
        .await
        .map(|v| {
            v.into_iter()
                .map(
...
pub async fn fail_task<T>(
    task: &TaskRequest<T>,
    body: impl Into<String>,
) -> Result<(), FocusError> {
    let body = body.into();
    warn!("Reporting failed task with id {}: {}", task.id, body);
    let result = beam_result::perm_failed(
...
pub async fn claim_task<T>(task: &TaskRequest<T>) -> Result<bool, FocusError> {
    let result = beam_result::claimed(
        CONFIG.beam_app_id_long.clone(),
        vec![task.from.clone()],
        task.id,
    );
