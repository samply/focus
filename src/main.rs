...
/// Project name reserved for the lens3 backend, which sends complete CQL rather
/// than a macro skeleton.
const LENS3_PROJECT: &str = "lens3";

/// Where the CQL about to be run came from, which decides whether it is
/// rewritten before being executed.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CqlSource {
    /// Generated by focus itself from an AST payload.
    Ast,
    /// Sent by a client as a macro skeleton: placeholders are expanded against
    /// the CQL snippets shipped with focus and a literal `define` is rejected.
    Templated,
    /// Sent by a trusted client as complete CQL and run verbatim.
    Raw,
}
...
async fn process_task(
    ...
    match CONFIG.endpoint_type {
        EndpointType::Blaze => {
            let mut cql_source = CqlSource::Templated;
            ...
            let query: CqlQuery = match serde_json::from_slice::<Language>(&data)? {
                Language::Cql(cql_query) => {
                    ...
                    {
                        if metadata.project == LENS3_PROJECT {
                            cql_source = CqlSource::Raw;
                        }
                        cql_query
                    } else {
...
async fn run_cql_query(
    ...
    let query = match cql_source {
        CqlSource::Templated => replace_cql_library(query.clone())?,
        CqlSource::Ast | CqlSource::Raw => query.clone(),
    };
