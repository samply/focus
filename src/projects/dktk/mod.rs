...
impl Project for Dktk {
    ...
    fn append_criterion_code_lists(&self, map: &mut HashMap<&str, Vec<&str>>) {
        map.extend([
            ...
            ("grading", vec!["gradingcs"]),
            ("metastases_present", vec!["jnucs"]),
            ("localization_metastases", vec!["fmlokalisationcs"]),
            ("procedure", vec!["Therapieart"]),
            ("medicationStatement", vec!["Therapieart"]),
            ("morphology", vec!["morph"]),
            ("sample_kind", vec!["specimentype"]),
            ("uiccstadium", vec!["uiccstadiumcs"]),
            ("observationMolecularMarkerName", vec!["molecularMarker"]),
            (
                "local_assessment_residual_tumor",
                vec!["Therapieart", "lokalebeurteilungresidualstatuscs"],
            ),
            ("responseOverTime", vec!["gesamtbeurteilungtumorstatuscs"]),
            (
                "localRegionalRecurrence",
                vec!["verlauflokalertumorstatuscs"],
            ),
            (
                "lymphNodeRecurrence",
                vec!["verlauftumorstatuslymphknotencs"],
            ),
            (
                "distantMetastases",
